...
const EditProductPage = () => {
  ...
  return (
    <main className="flex flex-1 flex-col">
      <PageHeader
        crumbs={["Cadastros", "Produtos", product?.name ?? "Editar"]}
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <FormItem>
                      ...
                      <div className="h-11 rounded-2 border border-line-2 bg-soft px-3 flex items-center justify-between">
                        <span className="text-[14px] text-ink tabular-nums">
                          {currentStock} {selectedUnit ?? ""}
                        </span>
                        <Link
                          href={`/estoque/alterar?productId=${product?.id ?? ""}`}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          {isLoading ? (
                            <Skeleton className="h-24 rounded-2" />
                          ) : (
                            <Textarea
                              rows={3}
